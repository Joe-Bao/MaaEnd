...
func RepoRoi(gridRowY, gridColX int) maa.Rect {
	x := RepoFirstX + gridColX*(SquareSize+GridInterval) + ToolTipCursorOffset
	y := RepoFirstY + gridRowY*(SquareSize+GridInterval) + ToolTipCursorOffset
	w := TooltipRoiScaleX
	h := TooltipRoiScaleY
	log.Trace().
...
func RepoSquarePos(gridRowY, gridColX int) maa.Rect {
	x := RepoFirstX + gridColX*(SquareSize+GridInterval)
	y := RepoFirstY + gridRowY*(SquareSize+GridInterval)
	w := SquareSize
	h := SquareSize
	log.Trace().
		Int("grid_row_y", gridRowY).
